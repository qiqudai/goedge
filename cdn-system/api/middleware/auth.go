...
func AuthRequired(requiredRole string) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if authHeader == "" {
			c.AbortWithStatusJSON(http.StatusUnauthorized, gin.H{"error": T("Authorization header missing")})
			return
		}
		...
		if len(parts) != 2 || parts[0] != "Bearer" {
			c.AbortWithStatusJSON(http.StatusUnauthorized, gin.H{"error": T("Invalid authorization format")})
			return
		}

		claims, err := utils.ParseToken(parts[1])
		if err != nil {
			c.AbortWithStatusJSON(http.StatusUnauthorized, gin.H{"error": T("Invalid or expired token")})
			return
		}
		...
		if requiredRole != "" && claims.Role != requiredRole {
			...
			c.AbortWithStatusJSON(http.StatusForbidden, gin.H{"error": T("Permission denied")})
			return
		}
...
func AuthAgent() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if token == "" {
			c.AbortWithStatusJSON(http.StatusUnauthorized, gin.H{"error": T("Authorization token missing")})
			return
		}
		...
		if config.App.AgentToken != "" {
			if token != config.App.AgentToken {
				c.AbortWithStatusJSON(http.StatusUnauthorized, gin.H{"error": T("Invalid agent token")})
				return
			}
