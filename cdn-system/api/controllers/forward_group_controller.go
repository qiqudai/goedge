...
func (ctrl *ForwardGroupController) List(c *gin.Context) {
	...
	if err := query.Find(&groups).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"code": 500, "msg": T("Failed to fetch groups")})
		return
	}
...
func (ctrl *ForwardGroupController) Create(c *gin.Context) {
	...
	if err := db.DB.Create(group).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"code": 500, "msg": T("Failed to create group")})
		return
	}
...
func (ctrl *ForwardGroupController) Update(c *gin.Context) {
	...
	if err := c.ShouldBindJSON(&req); err != nil || req.ID == 0 {
		c.JSON(http.StatusBadRequest, gin.H{"code": 400, "msg": T("id is required")})
		return
	}
	...
	if err := query.Updates(updates).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"code": 500, "msg": T("Failed to update group")})
		return
	}
...
func (ctrl *ForwardGroupController) Delete(c *gin.Context) {
	...
	if err := query.Delete(&models.ForwardGroup{}).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"code": 500, "msg": T("Failed to delete group")})
		return
	}
	services.BumpConfigVersion("forward_group", []int64{req.ID})
