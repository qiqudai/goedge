...
func (ctr *AgentController) GetConfig(c *gin.Context) {
	...
	if err != nil {
		...
		c.JSON(http.StatusInternalServerError, gin.H{"error": T("failed to generate config")})
		return
	}
...
func (ctr *AgentController) GetL2Nodes(c *gin.Context) {
	...
	if err := db.DB.Model(&models.Line{}).
		...
		c.JSON(http.StatusInternalServerError, gin.H{"error": T("failed to load node groups")})
		return
	}
	if len(groupIDs) == 0 {
		c.JSON(http.StatusOK, gin.H{"nodes": []gin.H{}})
...
func (ctr *AgentController) GetTasks(c *gin.Context) {
	...
	if err := db.DB.Where("enable = ? AND state IN ? AND (retry_at IS NULL OR retry_at <= ?)", true, []string{"waiting", "running"}, time.Now()).
		...
		c.JSON(http.StatusInternalServerError, gin.H{"error": T("failed to load tasks")})
		return
	}
...
func (ctr *AgentController) FinishTask(c *gin.Context) {
	...
	if err := db.DB.Where("id = ?", id).First(&task).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": T("failed to load task")})
		return
	}
	if strings.EqualFold(task.Type, "issue_cert") {
	...
	if err := db.DB.Model(&models.Task{}).Where("id = ?", id).Updates(updates).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": T("failed to update task")})
		return
	}
...
func buildTaskTitle(taskType string, state string) string {
	...
	switch taskType {
	case "refresh_url":
		label = i18n.T("task.refresh_url")
	case "refresh_dir":
		label = i18n.T("task.refresh_dir")
	case "preheat":
		label = i18n.T("task.preheat")
	}
	if state == "fail" {
		return label + i18n.T("task.failed_suffix")
	}
	return label + i18n.T("task.done_suffix")
}

func buildTaskContent(taskType string, state string, data string, ret string) string {
	result := i18n.T("task.exec_success")
	if state == "fail" {
		result = i18n.T("task.exec_failed")
	}
	if strings.TrimSpace(ret) != "" {
		result = result + i18n.T("task.reason_prefix") + ret
	}
	if strings.TrimSpace(data) == "" {
		return result
	}
	return result + i18n.T("task.url_prefix") + data
}
