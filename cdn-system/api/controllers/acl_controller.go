...
func (ctr *ACLController) List(c *gin.Context) {
	...
	if err := query.Count(&total).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": T("Failed to load ACL")})
		return
	}
...
func (ctr *ACLController) Create(c *gin.Context) {
	...
	if strings.TrimSpace(req.Name) == "" {
		c.JSON(http.StatusBadRequest, gin.H{"error": T("name is required")})
		return
	}
	...
	if err := db.DB.Create(&item).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": T("Failed to create")})
		return
	}
...
func (ctr *ACLController) Update(c *gin.Context) {
	...
	if err := db.DB.Omit("CreatedAt").Save(&item).Error; err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": T("Failed to update")})
		return
	}

	services.BumpConfigVersion("acl", []int64{id})

	c.JSON(http.StatusOK, gin.H{"code": 0, "message": T("updated")})
}
...
func (ctr *ACLController) Delete(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{"code": 0, "message": T("deleted")})
}
