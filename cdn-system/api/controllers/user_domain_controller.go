...
func (ctr *UserDomainController) CreateDomain(c *gin.Context) {
	...
	if strings.TrimSpace(req.Name) == "" {
		c.JSON(http.StatusBadRequest, gin.H{"msg": T("Domain name is required")})
		return
	}
	userID, _ := c.Get("userID")
	uid := userID.(int64)
	...
	if err := db.DB.Where("user_id = ? AND name = ?", uid, req.Name).First(&existing).Error; err == nil {
		c.JSON(http.StatusBadRequest, gin.H{"msg": T("Domain already exists")})
		return
	}
	...
	c.JSON(http.StatusOK, gin.H{"code": 0, "msg": T("Domain Created"), "data": domain})
}
...
func (ctr *UserDomainController) GetConfig(c *gin.Context) {
	...
	if err := db.DB.Preload("Origins").Where("id = ? AND user_id = ?", id, userID).First(&domain).Error; err != nil {
		c.JSON(http.StatusNotFound, gin.H{"msg": T("Domain Not Found")})
		return
	}
