...
func (ctr *FinanceController) Recharge(c *gin.Context) {
	...
	if req.UserID <= 0 || req.Amount <= 0 {
		c.JSON(http.StatusBadRequest, gin.H{"msg": T("Invalid user_id or amount")})
		return
	}
	...
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"msg": T("Recharge Failed")})
		return
	}

	c.JSON(http.StatusOK, gin.H{"msg": T("Recharge Successful")})
}
...
func (ctr *FinanceController) UserRecharge(c *gin.Context) {
	...
	if req.Amount <= 0 {
		c.JSON(http.StatusBadRequest, gin.H{"msg": T("Invalid amount")})
		return
	}
	...
	c.JSON(http.StatusOK, gin.H{"code": 0, "msg": T("Order Created")})
}
