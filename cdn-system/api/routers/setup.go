...
func Setup(r *gin.Engine) {
	...
	v1 := r.Group("/api/v1")
	{
		...
		admin.Use(middleware.OperationLog())
		{
			...
			admin.GET("/sites/batch/:id/progress", siteCtr.AdminBatchProgress)
			...
			admin.POST("/certs/wildcard", certCtr.WildcardCreate)
			admin.POST("/certs/batch_action", certCtr.BatchAction)
			// admin.POST("/certs/batch", certCtr.BatchIssue)
			// admin.GET("/certs/batch/:id/progress", certCtr.BatchProgress)
			admin.POST("/certs/reissue", certCtr.Reissue)
			admin.GET("/certs/:id/dns_challenge", certCtr.GetDNSChallenge)
			admin.POST("/certs/:id/verify_dns", certCtr.VerifyDNSChallenge)
			...
			admin.DELETE("/rules/cc/groups/:id", ruleCtr.DeleteCCRuleGroup)
			...
			admin.DELETE("/rules/cc/matchers/:id", ruleCtr.DeleteMatcher)
		...
		user.Use(middleware.OperationLog())
		{
			...
			user.POST("/certs/wildcard", certController.WildcardCreate)
			user.POST("/certs/batch_action", certController.BatchAction)
			// user.POST("/certs/batch", certController.BatchIssue)
			// user.GET("/certs/batch/:id/progress", certController.BatchProgress)
			...
			user.DELETE("/rules/cc/groups/:id", userRuleCtr.DeleteCCRuleGroup)
			...
			user.DELETE("/rules/cc/matchers/:id", userRuleCtr.DeleteMatcher)
