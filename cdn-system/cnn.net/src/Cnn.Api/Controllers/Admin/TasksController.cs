...
public sealed class TasksController : BaseApiController
{
    ...
    [HttpGet("usage")]
    public async Task<IActionResult> UsageAsync([FromQuery(Name = "user_id")] long? userId, CancellationToken cancellationToken)
    {
        ...
        var targetUserId = userId.GetValueOrDefault();
        if (targetUserId <= 0)
        {
            targetUserId = identity.UserId;
        }

        var result = await _service.GetUsageAsync(targetUserId, cancellationToken);
    ...
    public async Task<IActionResult> CreateAsync([FromBody] TaskCreateRequest request, CancellationToken cancellationToken)
    {
        ...
        var result = await _service.CreateAsync(request, identity.UserId, true, cancellationToken);
        return ToResponse(result);
    }
    ...
    {
        var result = await _service.GetAdminAsync(id, cancellationToken);
        return ToResponse(result);
    }
    ...
    public async Task<IActionResult> ResubmitAsync(long id, CancellationToken cancellationToken)
    {
        ...
        var result = await _service.ResubmitAsync(id, identity.UserId, true, cancellationToken);
        return ToResponse(result);
    }
