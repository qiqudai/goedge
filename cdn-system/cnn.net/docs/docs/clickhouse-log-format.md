...
## 5. Agent/API 交互 Trace（`agent_api_trace`）
来源：
- API WebSocket `/ws/agent` 双向消息追踪
- Agent HTTP 上报接口的请求追踪

事件类型：
- `node_events.event_type = agent_api_trace`

常见 payload 字段：
- 通用：`direction`、`channel`、`kind`、`trace_id`、`status_code`、`path`、`method`
- 任务/同步链路：`msg_id`、`task_id`、`task_type`、`sync_action`
- 限流摘要（新增）：`kind=rate_limit_summary`，字段包含：
  - `rate_limit_max_events_per_sec`
  - `window_second`
  - `accepted_events`
  - `dropped_events`

说明：
- 当开启 `debug-agent-api-trace-max-events-per-sec` 且瞬时超限时，超额 trace 会被丢弃。
- 系统会在下一秒窗口写入一条 `rate_limit_summary` 摘要事件，便于排障时观察丢弃规模。
