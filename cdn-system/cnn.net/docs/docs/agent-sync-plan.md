...
**传输载体**：已有 `services.UserPackageService.SyncUserPackage` 生成 `AgentPackageConfig` 的任务/Job，通过 WS `task_dispatch` 下发。  
...
## 流量超限ִÐÐÂ߼­£¨ÐÂÔö£©
- 数据Դ：agent Éϱ¨ access log -> ClickHouse `node_access_logs`，ͳ计 `sum(bytes)`，按 `host` 与 `site.domains` ƥÅ䡣
- 普ͨ¼ÆË㣺`ÌײÍÁ÷Á¿(GB)` vs `sum(bytes)`，可ʹ用 `tcp_traffic_factor`（system config_items）做修正。
- ִÐпأº`traffic_excceed_close_site`=1 才ִÐУ裨򲻸Ä `site.state`。
- 结果ӦÓãº
  1) 超限 -> `site.state=traffic_limit`，Nginx `return 509` + `error_page` ʹ用 `traffic_limit` ҳ。
  2) »ָ´ -> ֻ»ָ´µ±ǰ `state=traffic_limit` 的վµ㣬其它״̬²»¸ÉÈš£
- ͬ步：ÿ´θĶ¯ `site.state` 后调用 `BumpConfigVersion("site", siteIDs)`，agent 在下һ轮拉ȡ后ִ行 `nginx -s reload`，ȷ保生Ч。
