...
local function apply_filter(filter, rule, rule_id, host, ip, uri, action_override)
    if not filter then
        ...
        if action == "block" or action == "ipset" then
            block_request(build_cc_reason("cc_block", rule_id, rule, nil, "action=" .. action), 403)
        end
        return "continue"
    end
    ...
    if filter_type == "url_auth" then
        ...
        block_request(build_cc_reason("cc_url_auth", rule_id, rule, filter, detail or rate_detail), 403)
    end
    ...
    if guard.is_guard_filter(filter_type) then
        ...
        if non_browser and extra.block_non_browser ~= false then
            block_request(build_cc_reason("cc_non_browser", rule_id, rule, filter, non_browser_detail), 403)
        end
        local exceeded, detail = rate_exceeded(filter, host, ip, uri)
        if should_force_guard(filter) or exceeded then
            if not guard.ensure_passed(filter, host, ip) then
    ...
    if filter_type == "" or filter_type == "req_rate" or filter_type == "block" then
        if filter_type == "block" and (tonumber(filter.max_req or 0) <= 0 and tonumber(filter.max_req_per_uri or 0) <= 0) then
            block_request(build_cc_reason("cc_block", rule_id, rule, filter, "config=block"), 403)
        end
        local exceeded, detail = rate_exceeded(filter, host, ip, uri)
        if exceeded then
...
local function execute_rule(rule, host, ip, uri, config, rule_id, ctx)
    ...
    if rule.filter_id and rule.filter_id > 0 then
        filter = resolve_filter(config, rule.filter_id)
    elseif rule.filter1_id and rule.filter1_id > 0 then
        filter = resolve_filter(config, rule.filter1_id)
    elseif rule.filter2_id and rule.filter2_id > 0 then
        filter = resolve_filter(config, rule.filter2_id)
