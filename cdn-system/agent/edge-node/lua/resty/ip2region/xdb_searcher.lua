...
function xdb:search(ip_bytes)
    ...
    local il0 = bit.band(string.byte(ip_bytes, 1), 0xFF)
    local il1 = bit.band(string.byte(ip_bytes, 2), 0xFF)
    ...
    local bytes, d_bytes = version.bytes, bit.lshift(version.bytes, 1)
...
function le_get_uint32(buff, idx)
    ...
    local i2 = bit.lshift(string.byte(buff, idx+1), 8)
    local i3 = bit.lshift(string.byte(buff, idx+2), 16)
    local i4 = bit.lshift(string.byte(buff, idx+3), 24)
...
function xdb.verify(db_path)
    ...
    local max_file_ptr = math.pow(2, runtime_ptr_bytes * 8) - 1
...
function _parse_ipv4_addr(v4_str)
    ...
    return string.char(tbl_unpack(bytes)), nil
end
...
function _parse_ipv6_addr(v6_str)
    ...
    while i <= length do
        ...
        bytes[offset] = bit.band(bit.rshift(v, 8), 0xFF)
        bytes[offset+1] = bit.band(v, 0xFF)
        offset = offset + 2
    ...
    return string.char(tbl_unpack(bytes))
end
...
function _ipv6_to_string(ip_bytes, compress)
    ...
    for i = 1, #ip_bytes, 2 do
        hex = bit.bor(bit.lshift(string.byte(ip_bytes, i), 8), string.byte(ip_bytes, i + 1))
