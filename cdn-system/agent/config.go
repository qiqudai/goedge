...
func writeHTTPConfig(cfg edgeConfig) error {
	...
	if blockUnbound {
		...
		httpPorts := collectHTTPPorts(cfg.Domains, defaultListen80)
		for _, port := range httpPorts {
			writeDefaultHTTPServer(&b, port, errorPages, errorPageDir, 404)
		}
	}
...
func fallbackCertPath() string {
	...
	certPath := filepath.Join(WorkDir, "cert", "fallback.pem")
...
func fallbackKeyPath() string {
	...
	keyPath := filepath.Join(WorkDir, "cert", "fallback.key")
