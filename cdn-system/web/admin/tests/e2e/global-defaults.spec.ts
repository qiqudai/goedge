...
test('global defaults apply to new sites', async ({ page, playwright }) => {
  ...
  const token = await loginAdminToken(apiBase)
  ...
  const agentConfigCandidates = [
    ...
    path.resolve(process.cwd(), '..', '..', 'agent', 'agent.json')
  ].filter(Boolean) as string[]
  let agentConfig: any = null
  for (const candidate of agentConfigCandidates) {
    try {
      if (fs.existsSync(candidate)) {
        agentConfig = JSON.parse(fs.readFileSync(candidate, 'utf-8'))
        break
      }
    } catch {
  ...
  const agentNodeId = Number(process.env.AGENT_NODE_ID || agentConfig?.node_id || 0)
  const agentApi = agentToken
    ? await playwright.request.newContext({
      baseURL: apiBase,
      extraHTTPHeaders: { Authorization: `Bearer ${agentToken}` }
    })
    : null
  const agentNodeGroupId = agentToken && agentNodeId ? await getNodeGroupForNode(api, agentNodeId) : 0
  const canCheckAgent = !!agentToken && agentNodeId > 0 && agentNodeGroupId > 0
  ...
  for (const item of siteDefaultCases) {
    await test.step(`全局默认: ${item.label}`, async () => {
      ...
      const { id } = await createSiteByApi(api, domain, originAddress)
      try {
    ...
    for (const type of types) {
      for (const field of fields) {
        await test.step(`缓存配置: ${type} ${field}`, async () => {
          ...
          const { id } = await createSiteByApi(api, domain, originAddress, type)
          try {
            const site = await getSiteDetail(api, id)
