<template>
  <el-form v-loading="loading" label-width="150px" class="config-form">
    ...
    <el-table
      ref="cacheTableRef"
      :data="form.cacheRules"
      border
      class="config-table"
      @selection-change="handleCacheSelectionChange"
    >
...
const selectedCacheRules = ref([])
const selectedOriginHeaders = ref([])

const cacheSelectionKey = computed(() => 'table-selection:global-cache-rules')
const headerSelectionKey = computed(() => 'table-selection:global-origin-headers')
...
const saveSelection = (key, rows) => {
  if (!key) return
  try {
    const ids = (rows || []).map(getRowKey).filter(Boolean)
...
const restoreSelection = (key, tableRef, rows) => {
  if (!key) return
  let saved = []
  try {
  ...
  rows.forEach((row) => {
    if (saved.includes(getRowKey(row))) {
      table.toggleRowSelection(row, true)
    }
  })
}

const handleCacheSelectionChange = (rows) => {
  selectedCacheRules.value = rows
  saveSelection(cacheSelectionKey.value, rows)
}

const handleHeaderSelectionChange = (rows) => {
  selectedOriginHeaders.value = rows
  saveSelection(headerSelectionKey.value, rows)
}
...
watch(
  ...
  () => {
    nextTick(() => {
      restoreSelection(cacheSelectionKey.value, cacheTableRef, form.cacheRules)
    })
  },
  { deep: true }
)
...
watch(
  ...
  () => {
    nextTick(() => {
      restoreSelection(headerSelectionKey.value, headerTableRef, form.originHeaders)
    })
  },
  { deep: true }
)
...
onBeforeUnmount(() => {
  sessionStorage.removeItem(cacheSelectionKey.value)
  sessionStorage.removeItem(headerSelectionKey.value)
})
</script>
