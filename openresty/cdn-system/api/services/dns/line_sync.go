...
func SyncLineRecords(groupID int64, lineID, lineName, action string, nodeIPIDs []int64) error {
	...
	for providerID, count := range domainCounts {
		...
		log.Printf("[DNS] sync start provider=%s group=%d line=%s action=%s nodes=%d domains=%d", state.api.Type, groupID, lineID, action, len(nodes), count)
	}
	errs := make([]string, 0)
	...
	for _, domain := range domains {
		...
		if deleteAll {
			...
			if deleter, ok := state.provider.(LineRecordDeleter); ok {
				if err := deleter.DeleteRecordsByLine(root, record); err != nil {
	...
	if len(errs) > 0 {
		...
		log.Printf("[DNS] sync failed group=%d line=%s action=%s errors=%s", groupID, lineID, action, msg)
		return errors.New(msg)
	}
	log.Printf("[DNS] sync success group=%d line=%s action=%s", groupID, lineID, action)
	return nil
}
