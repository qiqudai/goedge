...
export function createLoadingTracker(page: Page) {
  ...
  const expectLoadingOnRequest = async (label: string, action: () => Promise<void>) => {
    const requestPromise = page
      .waitForRequest(req => req.url().includes('/api/v1/'), { timeout: 1200 })
      .catch(() => null)
    ...
    try {
      await page.locator('.global-loading-overlay').waitFor({ state: 'visible', timeout: 800 })
    } catch {
      missing.push(label)
    }
    await page.locator('.global-loading-overlay').waitFor({ state: 'hidden', timeout: 1000 }).catch(() => null)
  }
