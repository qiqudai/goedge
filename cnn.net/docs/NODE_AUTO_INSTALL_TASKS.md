...
> 目标：在创建新节点时支持 SSH 认证与自动安装，自动生成 agent.json（含 token/node_id），并立即启动最新 cdn-agent。
...
- `ssh_key`：私钥（不在响应中回传）
- `work_dir`：安装目录记录（默认 `/www/node/app`；agent.json 不再写 `work_dir`，运行时以可执行文件所在目录为准）
...
**改动点**：
- Create/Update 使用请求结构体接收 SSH/工作目录字段（仅记录安装目录）
...
**伪代码**：
```
workDir := normalizeWorkDir(req.WorkDir) // 可执行文件所在目录
mkdir -p workDir
scp cdn-agent -> workDir/cdn-agent (chmod 755)
write agent.json -> workDir/agent.json // 不写 work_dir
