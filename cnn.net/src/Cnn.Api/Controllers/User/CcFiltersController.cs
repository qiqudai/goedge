...
[ApiController]
[Route("api/v1/user/rules/cc/filters")]
public sealed class CcFiltersController : ControllerBase
{
    private readonly ICcFilterService _service;
    ...
    public CcFiltersController(
        ICcFilterService service,
    ...
    [HttpPost]
    public async Task<IActionResult> CreateAsync([FromBody] CcFilterUpsertRequest request, CancellationToken cancellationToken)
    {
    ...
    [HttpPut("{id:long}")]
    public async Task<IActionResult> UpdateAsync(long id, [FromBody] CcFilterUpsertRequest request, CancellationToken cancellationToken)
    {
        var (userId, error) = await EnsureCustomCcRuleAllowedAsync(cancellationToken);
        if (error != null)
        {
    ...
    public async Task<IActionResult> DeletePreviewAsync(long id, CancellationToken cancellationToken)
    {
        ...
        var result = await _deletionPreviewService.PreviewAsync(ResourceTypes.CcFilter, id, cancellationToken);
    ...
    private async Task<IActionResult> RequestDeleteAuthorizedAsync(long id, long userId, CancellationToken cancellationToken)
    {
        ...
        var result = await _resourceDeleteRequestService.RequestDeleteAsync(
            DeleteRequestCommandFactory.Create(ResourceTypes.CcFilter, id, userId, userId),
            cancellationToken);
        if (result.Success)
        {
