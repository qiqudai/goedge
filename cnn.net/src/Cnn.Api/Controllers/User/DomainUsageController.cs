...
public sealed class DomainUsageController : BaseApiController
{
    ...
    [HttpGet]
    public async Task<IActionResult> GetAsync([FromQuery(Name = "user_package_id")] long? userPackageId, CancellationToken cancellationToken)
    {
        ...
        {
            resolvedPackageId = await _service.FindDefaultUserPackageIdAsync(userId);
            if (resolvedPackageId <= 0)
            {
        ...
        var result = await _service.GetUsageAsync(userId, resolvedPackageId, cancellationToken);
        if (!result.Success)
        {
