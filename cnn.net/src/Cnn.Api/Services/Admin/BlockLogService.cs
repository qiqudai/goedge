...
public interface IBlockLogService
{
    ...
    Task<ServiceResult<bool>> UnblockIpAsync(string ip, long? userId, bool isAdmin, CancellationToken cancellationToken);
    Task<ServiceResult<bool>> UnblockBatchAsync(IReadOnlyList<string> ips, long? userId, bool isAdmin, CancellationToken cancellationToken);
    Task<ServiceResult<bool>> UnblockSiteAsync(IReadOnlyList<long> siteIds, long? userId, bool isAdmin, CancellationToken cancellationToken);
}
...
public sealed class BlockLogService : IBlockLogService
{
    ...
    public Task<ServiceResult<bool>> UnblockIpAsync(string ip, long? userId, bool isAdmin, CancellationToken cancellationToken)
    {
        ip = ip?.Trim() ?? string.Empty;
        if (string.IsNullOrWhiteSpace(ip))
        {
            return Task.FromResult(ServiceResult<bool>.Fail(ErrorCodes.InvalidParam, "invalid_param"));
        }

        return UnblockBatchAsync(new[] { ip }, userId, isAdmin, cancellationToken);
    }

    public async Task<ServiceResult<bool>> UnblockBatchAsync(IReadOnlyList<string> ips, long? userId, bool isAdmin, CancellationToken cancellationToken)
    {
        var list = ips?
        ...
        var (index, hostFilter, _) = await ResolveFiltersAsync(new BlockLogQuery { Type = "ip", Keyword = string.Empty }, userId, isAdmin, cancellationToken, ignoreIpFilter: true);
        if (hostFilter == null)
        {
            return ServiceResult<bool>.Fail(ErrorCodes.PermissionDenied, "permission_denied");
        }

        var cfg = ClickHouseHttpHelper.ResolveConfig(_configuration);
        if (cfg == null)
        {
            return ServiceResult<bool>.Fail(ErrorCodes.InternalError, "clickhouse_unavailable");
        }

        var ipClause = "remote_addr IN (" + string.Join(",", list.Select(ClickHouseHttpHelper.QuoteString)) + ")";
        var where = BuildUnblockWhere(hostFilter, ipClause);
        var deleted = await ExecuteMutationAndVerifyAsync(cfg, where, cancellationToken);
        return deleted
            ? ServiceResult<bool>.Ok(true)
            : ServiceResult<bool>.Fail(ErrorCodes.InternalError, "unblock_verify_failed");
    }

    public async Task<ServiceResult<bool>> UnblockSiteAsync(IReadOnlyList<long> siteIds, long? userId, bool isAdmin, CancellationToken cancellationToken)
    {
        var ids = siteIds?.Where(id => id > 0).Distinct().ToList() ?? new List<long>();
        if (ids.Count == 0)
        {
            return ServiceResult<bool>.Fail(ErrorCodes.InvalidParam, "invalid_param");
        }
        ...
        if (!isAdmin && userId.GetValueOrDefault() <= 0)
        {
            return ServiceResult<bool>.Fail(ErrorCodes.PermissionDenied, "permission_denied");
        }

        var siteFilters = ids
            .Select(id => index.SiteFilters.TryGetValue(id, out var f) ? f : null)
            .Where(f => f != null && !f.Empty)
            .ToList();
        ...
        var hostConditions = siteFilters
            .Select(f => f!.BuildHttpCondition())
        ...
        var hostClause = "(" + string.Join(" OR ", hostConditions) + ")";
        var where = BuildUnblockWhere(null, hostClause);
        var deleted = await ExecuteMutationAndVerifyAsync(cfg, where, cancellationToken);
        return deleted
            ? ServiceResult<bool>.Ok(true)
            : ServiceResult<bool>.Fail(ErrorCodes.InternalError, "unblock_verify_failed");
    ...
    private static string BuildUnblockWhere(HostFilter? hostFilter, string targetClause)
    {
        var conditions = new List<string> { BuildStatusCondition(), targetClause };
        if (hostFilter != null && !hostFilter.Empty)
        {
            conditions.Add(hostFilter.BuildHttpCondition());
        }
    ...
    private static async Task<bool> ExecuteMutationAndVerifyAsync(ClickHouseHttpConfig cfg, string where, CancellationToken cancellationToken)
    {
        var sql = $"ALTER TABLE node_access_logs DELETE WHERE {where} SETTINGS mutations_sync = 1";
        var ok = await ClickHouseHttpHelper.ExecuteAsync(cfg, sql, cancellationToken);
        if (!ok)
        {
            return false;
        }
        ...
        {
            var remain = await QueryTotalAsync(cfg, $"SELECT count() AS total FROM node_access_logs WHERE {where}", cancellationToken);
            if (remain <= 0)
            {
                return true;
            }
